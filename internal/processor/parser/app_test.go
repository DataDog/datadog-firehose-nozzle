	...
	Context("metric evaluation test", func() {
		It("parses an event properly and adds metadata if configured using cloudfoundry client", func() {
			...
			for _, metric := range metrics {
				...
				Expect(metric.MetricValue.Tags).To(ContainElement("space-org-label:space-org-label-space-value"))
				Expect(metric.MetricValue.Tags).To(ContainElement("space-label:space-label-value"))
				Expect(metric.MetricValue.Tags).To(ContainElement("org-label:org-label-value"))
				...
				Expect(metric.MetricValue.Tags).To(ContainElement("space-org-annotation:space-org-annotation-space-value"))
				Expect(metric.MetricValue.Tags).To(ContainElement("space-annotation:space-annotation-value"))
				Expect(metric.MetricValue.Tags).To(ContainElement("org-annotation:org-annotation-value"))
		...
		It("parses an event properly and adds metadata if configured using cluster agent client", func() {
			config.NozzleConfig.EnableMetadataCollection = true
			config.NozzleConfig.MetadataKeysBlacklist = []*regexp.Regexp{regexp.MustCompile("blacklisted.*")}
			a, err := NewAppParser(nil, fakeDCAClient, 5, 10, log, []string{}, "env_name")
			Expect(err).To(BeNil())
			Eventually(a.AppCache.IsWarmedUp).Should(BeTrue())
